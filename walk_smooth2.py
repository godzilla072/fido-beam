...
FREQUENCY = 0.5  # 0.5 Controls speed of walking

# Predefined range of motion offsets
BOTTOM_SWING_FORWARD_OFFSET = 30.0
TOP_LIFT_UP_OFFSET = 10.0

######### FRONT #########

FRONT_LEFT_BOTTOM = 180.0   # Decreasing this value makes the leg reach more forward
FRONT_LEFT_TOP = 200.0      # Increasing this value causes the leg to push more off the ground

FRONT_RIGHT_BOTTOM = 165.0  # Increasing this value makes the leg reach more forward
FRONT_RIGHT_TOP = 170.0     # Decreasing this value causes the leg to push more off the ground
...
BACK_RIGHT_TOP = 155.0      # Decreasing this value causes the leg to push more off the ground
...
def walk_smoothly():
    """Animate walking process using sine and cosine functions for a crawl gait."""
    print("\n### Starting smooth walking mode ###\n")

    # Phase shifts for a crawl gait
    phase_shifts = {
        'Front Left': 0.0,
        'Front Right': pi/2,
        'Back Right': pi,
    ...
    try:
        while True:
            ...
            for leg in LEGS:
                ...
                # Use the new phase shifts for a crawl gait
                phase_shift = phase_shifts[leg_name]
                phase = t + phase_shift

                # Top servo: controls leg lift. Using cosine:
                # cos(phase) = 1 at phase=0 (leg down), and -1 at phase=pi (leg fully up).
                # We'll invert this to have leg lifted at the midpoint:
                # (cos(phase) goes from +1 down to -1) - You may tweak if needed.
                top_angle = top_amplitude * cos(phase) + top_offset

                # Bottom servo: controls forward/back swing. Using sine:
                # At phase=0, sin=0. As phase increases, the leg moves forward when lifted.
                # With the crawl gait, each leg should swing forward during its lift phase.
                bottom_angle = bottom_amplitude * sin(phase) + bottom_offset
